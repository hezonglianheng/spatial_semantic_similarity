...
PROMPTS = [
    r'在句子"{sentence1}"中，"{target1}"的位置或方向是：',
    r'在句子"{sentence2}"中，"{target2}"的位置或方向是：',
    r'在句子"{sentence1}"中，"{target1}"相对于"{reference1}"的位置或方向是：',
    r'在句子"{sentence2}"中，"{target2}"相对于"{reference2}"的位置或方向是：',
]
...
def main():
    ...
    for p in range(0, len(PROMPTS), 2):
        ...
        df.to_csv(os.path.join(args.output_dir, f"similarities_spatial_prompt{prompt_group}_{args.model_alias}.csv"), index=False)
    ...
    # 将所有组的结果统一写入同一个文件
    with open(os.path.join(args.output_dir, f"results_spatial_prompt_{args.model_alias}.txt"), "w") as f:
