...
def add_prompt(sentence: str, prompt_idx: int = 0) -> str:
    assert 0 <= prompt_idx < len(PROMPTS), "prompt的索引值超出范围"
    return PROMPTS[prompt_idx].format(sentence=sentence)
...
def main():
    ...
    # 为每个句子添加prompt信息
    data.sentences = [(add_prompt(s1), add_prompt(s2)) for (s1, s2) in data.sentences]
