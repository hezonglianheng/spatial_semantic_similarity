#!/bin/bash
# =============================================================================
# 批量运行 spatial_word_embedding.py — 支持指定多个模型及其别名
# =============================================================================
# 用法:
#   ./run_spatial_word_embedding_batch.sh [选项]
#
# 指定模型的方式 (优先级从高到低):
#   1. --models "model1::alias1 model2::alias2 ..."  (命令行, 空格分隔)
#   2. 直接编辑下方 DEFAULT_MODELS 数组
#
# 示例:
#   ./run_spatial_word_embedding_batch.sh
#   ./run_spatial_word_embedding_batch.sh --models "bert-base-chinese::bert-chinese Qwen/Qwen2-0.5B::qwen2-0.5b"
#   ./run_spatial_word_embedding_batch.sh --models "bert-base-chinese::bert-chinese" -d my_data.json -o ./results
#   ./run_spatial_word_embedding_batch.sh --save-diff-vectors
# =============================================================================
...
选项:
  ...
  --save-diff-vectors            保存每个样本的差异区域向量及原始数据到 .pkl 文件
  -h, --help                     显示此帮助信息
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --save-diff-vectors)
            SAVE_DIFF_VECTORS="--save_diff_vectors"; shift ;;
        -h|--help)
            usage ;;
        *)
            echo "[错误] 未知选项: $1"
...
# ---------- 打印批量运行信息 ----------
echo "============================================"
echo "  批量运行 spatial_word_embedding.py"
echo "============================================"
echo "  数据文件:     ${DATA_FILE}"
...
for entry in "${MODEL_ENTRIES[@]}"; do
    ...
    if python spatial_word_embedding.py \
        --data_file "${DATA_FILE}" \
        --model_name_or_path "${MODEL_PATH}" \
        --model_alias "${MODEL_ALIAS}" \
        --output_dir "${OUTPUT_DIR}" \
        ${SAVE_DIFF_VECTORS}; then
        echo ""
